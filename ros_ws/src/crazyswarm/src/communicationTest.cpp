...
//        m_serviceSetGroupMask = n.advertiseService(tf_prefix + "/getServerPosSetPoint", &CrazyflieROS::getPosSetPoint, this);
...
//        std::cout<<"size: "<<m_cfs.size()<<std::endl;
...
//            sp_states.back().yaw = 0.0f;
                sp_states.back().thrust = 0.0f;
            }
        }
//        std::cout<<"size: "<<sp_states.size()<<std::endl;
...
//        }
        ...
        {  //block: variables in the {} ends when exiting it
            auto start = std::chrono::high_resolution_clock::now();
            if (m_sendAttSp){
                if(m_beginPosSp)
                {
...
//                std::cout<<"send setpoint"<<std::endl;
...
//            m_latency.broadcasting = elapsedSeconds.count();
    ...
    {
        ros::NodeHandle nl("~");
//        std::cout<<"run slow group"<<std::endl;
        bool enableLogging;
    ...
    void Groupcontrol(int id,CrazyflieBroadcaster::AttSetPts& sp_state) {
        ...
        vec4ftmp = cf->m_controller.control_nonLineaire(cf->m_currentPosition,
    ...
    void getGroupCurPos(int id, float x, float y, float z)
    {
        m_CrazyflieIdMap[id]->giveCurrentPos(x,y,z);
//        ROS_INFO("after!");
    }
...
public:
            ...
            , m_getPosSetPoint()
    {
        ...
        m_getPosSetPoint = nh.advertiseService("getServerPosSetPoint",&CrazyflieServer::getPosSetPoint, this);
...
//        m_subscribeVirtualInteractiveObject = nh.subscribe("virtual_interactive_object", 1, &CrazyflieServer::virtualInteractiveObjectCallback, this);
    ...
    void runFast()
    {
        ...
        while (ros::ok() && !m_isEmergency) {
            ...
            for (auto group : m_groups) {
//                std::cout<<"group run fast in server"<<std::endl;
...
private:
    ...
    {
        ROS_INFO("Get Position Setpoint!");
...
//
            ...
            std::set<int>& channels)
    {
        ...
        nGlobal.getParam("crazyflies", crazyflies);
        ROS_ASSERT(crazyflies.getType() == XmlRpc::XmlRpcValue::TypeArray);
//        std::cout<<"crazyflies size read channels: "<<crazyflies[0]["channel"]<<std::endl;
        channels.clear();
        for (int32_t i = 0; i < crazyflies.size(); ++i) {
            ROS_ASSERT(crazyflies[i].getType() == XmlRpc::XmlRpcValue::TypeStruct);
            XmlRpc::XmlRpcValue crazyflie = crazyflies[i];
...
private:
    ...
    ros::ServiceServer m_serviceUpdateParams;
    ros::ServiceServer m_getPosSetPoint;
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "communication_test");
    ...
    std::set<int> cfIds;
    std::cout<<"crazyflies size: "<<crazyflies.size()<<std::endl;
    for (int32_t i = 0; i < crazyflies.size(); ++i)
    {
        ROS_ASSERT(crazyflies[i].getType() == XmlRpc::XmlRpcValue::TypeStruct);
        XmlRpc::XmlRpcValue crazyflie = crazyflies[i];
    ...
    server.run();
//    CrazyflieBroadcaster m_cfbc("radio://0/80/2M/FFE7E7E7E7");
//    while(1){
//        std::vector<CrazyflieBroadcaster::AttSetPts> sp_states;
//
//        sp_states.resize(sp_states.size() + 1);
